...
export const ReportList: React.FC<ReportListProps> = ({
  ...
  const lateStats = useMemo(() => {
    const lateList = submissions.filter(s => s.isLate);
    const lateTeachers = Array.from(new Set(lateList.map(s => s.authorName)));
    return { lateCount: lateList.length, lateTeachers, lateList };
  }, [submissions]);
  ...
  const handleWaiveAllLate = async () => {
    setIsWaivingLate(true);
    try {
      const res = await waiveAllLateStatus();
      if (res.success) {
        setActionMessage(`Đã miễn trừ thành công cho ${res.waivedCount} bài nộp trễ! Tất cả báo cáo đã được chuyển sang trạng thái "Đúng hạn".`);
        setTimeout(() => setActionMessage(null), 6000);
      }
    } catch (e) {
      console.error(e);
    } finally {
      setIsWaivingLate(false);
      setShowWaiveConfirm(false);
    }
  };

  const handleWaiveSingleLate = async (id: string, name: string) => {
    try {
      const res = await waiveLateStatus(id);
      if (res.success) {
        setActionMessage(`Đã xóa trạng thái nộp trễ cho Thầy/Cô ${name} và chuyển sang "Đúng hạn"!`);
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Waive Late Notice & Quick Action */}
      {(isAdmin || isPrincipal) && lateStats.lateCount > 0 && (
        <div className="bg-rose-50/80 border border-rose-200 p-3.5 rounded-2xl flex flex-wrap items-center justify-between gap-3 shadow-2xs">
          <div className="flex items-center gap-2.5">
            <Clock className="w-5 h-5 text-rose-600 shrink-0" />
            <div>
              <p className="text-xs font-bold text-rose-950">
                Có {lateStats.lateCount} bài nộp bị ghi nhận trễ hạn ({lateStats.lateTeachers.slice(0, 4).join(', ')}{lateStats.lateTeachers.length > 4 ? ` và ${lateStats.lateTeachers.length - 4} Thầy/Cô khác` : ''})
              </p>
              <p className="text-[11px] text-rose-700 mt-0.5">
                Do sự cố nghẽn mạng/hạn ngạch hệ thống lúc cao điểm, Thầy có thể bấm nút bên phải để miễn trừ toàn bộ và chuyển các bài này về trạng thái "Đúng hạn".
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs flex items-center gap-1.5 transition cursor-pointer shadow-xs active:scale-98"
            title="Chuyển tất cả báo cáo trễ hạn về Đúng hạn do sự cố hệ thống"
          >
            <CheckCircle2 className="w-3.5 h-3.5" />
            <span>{isWaivingLate ? 'Đang cập nhật...' : `Chuyển ${lateStats.lateCount} Bài Trễ Sang Đúng Hạn`}</span>
          </button>
        </div>
      )}
                      ...
                      <td className="py-3 px-4 whitespace-nowrap">
                        {sub.isLate ? (
                          <div className="flex items-center gap-1.5">
                            ...
                            {(isAdmin || isPrincipal) && (
                              <button
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleWaiveSingleLate(sub.id, sub.authorName);
                                }}
                                className="px-2 py-0.5 rounded-md text-[10px] font-bold bg-amber-100 hover:bg-amber-200 text-amber-900 border border-amber-300 transition cursor-pointer"
                                title="Xóa ghi nhận trễ hạn do sự cố hệ thống và chuyển thành đúng hạn"
                              >
                                Miễn trừ
                              </button>
                            )}
                          ...
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200">
                            ...
                            <span>{sub.lateWaived ? 'Đúng hạn (Miễn trừ)' : 'Đúng hạn'}</span>
                          </span>
                        )}
                      </td>
                ...
                <div>
                  <div className="flex items-center justify-between gap-2 mb-2">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          {(isAdmin || isPrincipal) && (
                            <button
                              ...
                              className="px-1.5 py-0.5 rounded-md text-[10px] font-bold bg-amber-100 hover:bg-amber-200 text-amber-900 border border-amber-300 transition cursor-pointer"
                              title="Miễn trừ trễ hạn"
                            >
                              Miễn trừ
                            </button>
                          )}
                      ...
                      {!sub.isLate && sub.lateWaived && (
                        <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 flex items-center gap-1">
                          <CheckCircle2 className="w-3 h-3" />
                          <span>Đúng hạn (Miễn trừ)</span>
                        </span>
                      )}
                      {isAuthorOrAdmin && (
                        <button
      ...
      {/* Waive All Late Confirmation Modal */}
      {showWaiveConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl border border-slate-200 animate-in fade-in zoom-in-95 duration-150">
            ...
            <h3 className="text-base font-bold text-slate-900 mb-2">
              Xác nhận miễn trừ nộp trễ hạn?
            </h3>
            
            <p className="text-xs text-slate-600 leading-relaxed mb-4">
              Thầy đang chọn chuyển <strong>{lateStats.lateCount} bài nộp</strong> đang bị ghi nhận "Trễ hạn" sang trạng thái <strong>"Đúng hạn"</strong>. Toàn bộ số liệu thống kê sẽ được cập nhật lại ngay lập tức.
            </p>

            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-[11px] text-emerald-900 mb-5 flex items-start gap-2">
              <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" />
              <span>Hành động này áp dụng cho các Thầy/Cô nộp báo cáo gặp sự cố nghẽn mạng/hạn ngạch hệ thống trong đợt vừa qua.</span>
            </div>
            ...
            <div className="flex items-center justify-end gap-3">
              ...
              <button
                type="button"
                onClick={handleWaiveAllLate}
                disabled={isWaivingLate}
                className="px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold transition cursor-pointer flex items-center gap-1.5 shadow-sm shadow-emerald-600/30"
              >
                {isWaivingLate ? (
                  <>
                    <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                  ...
                  <>
                    <CheckCircle2 className="w-3.5 h-3.5" />
                    <span>Xác Nhận Chuyển Thành Đúng Hạn</span>
                  </>
                )}
              </button>
            </div>
